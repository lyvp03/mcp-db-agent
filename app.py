...
from registry import DatabaseSource, get_default_source, list_sources, upsert_source
from schema_service import refresh_schema_cache
from upload_importer import (
    admin_connection_uri,
    create_database,
    database_uri_for_name,
...
with st.sidebar:
    ...
    if st.button("Create / Import", use_container_width=True):
        if not uploaded_files:
            ...
            try:
                ...
                base_uri = base_database_uri()
                admin_uri = admin_connection_uri(base_uri)
                target_uri = database_uri_for_name(base_uri, db_slug)

                create_database(admin_uri, db_slug)
                ...
                source = DatabaseSource(
                    ...
                    name=source_name,
                    db_type="postgres",
                    database_uri=target_uri,
                    schema_name="public",
                )
                upsert_source(source, make_default=make_default)
                asyncio.run(refresh_schema_cache(source))
                st.success(
                    "Imported files into database "
                    f"`{db_slug}`: {', '.join(f'`{name}`' for name in imported_names)}."
                )
    ...
    if not available_sources:
        ...
        for source in available_sources:
            is_default = default_source is not None and source.source_id == default_source.source_id
...
selected_name = st.selectbox(
    ...
    index=0 if source_map else None,
)
